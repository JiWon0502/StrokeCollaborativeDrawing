...
def make_graph(sketch, graph_num=30, graph_picture_size=128, padding=0, thickness=5,
    ...
    adj_matrix = np.eye(graph_num, dtype=float) * 0.5  # np.float -> float# (graph_num, graph_num)
...
def scale_sketch(sketch, size=(448, 448)):
    ...
    if h >= w:
        sketch_normalize = sketch / np.array([[h, h, 1]], dtype=float) # np.float -> float
    else:
        sketch_normalize = sketch / np.array([[w, w, 1]], dtype=float) # np.float -> float
    sketch_rescale = sketch_normalize * np.array([[size[0], size[1], 1]], dtype=float) # np.float -> float
