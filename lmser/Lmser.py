...
class SketchesDataset:
    ...
    def make_batch(self, batch_size):
        ...
        if hp.use_cuda:
            batch = torch.from_numpy(np.stack(sketches, 1)).to(device).float()  # (Nmax, batch_size, 5)
            graphs = torch.from_numpy(np.stack(graphs, 0)).to(device).float()  # (batch_size, len, 5)
            adjs = torch.from_numpy(np.stack(adjs, 0)).to(device).float()
...
class Model:
    def __init__(self):
        if hp.use_cuda:
            ...
            self.encoder: nn.Module = myencoder(hps=hp).to(device)
            self.decoder: nn.Module = DecoderRNN().to(device)
        else:
            self.encoder: nn.Module = EncoderGCN(hp.graph_number, hp.graph_picture_size, hp.out_f_num, hp.Nz,
                                                 bias_need=False)
    ...
    def make_target(self, batch, lengths):
        ...
        """
        if hp.use_cuda:
            eos = torch.stack([torch.Tensor([0, 0, 0, 0, 1])] * batch.size()[1]).to(device).unsqueeze(
    ...
    def kullback_leibler_loss(self):
        ...
        if hp.use_cuda:
            KL_min = torch.Tensor([hp.KL_min]).to(device).detach()
        else:
