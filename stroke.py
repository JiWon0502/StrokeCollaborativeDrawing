...
for a in range(100):
    ...
    if len(points) < q:
        ...
        points = points[:q, :] # 0 ~ (q - 1)까지 저장 (총 q개의 stroke 저장)
        list = []
        list.append(points.reshape(points.shape))
        reshaped_list = np.array(list)
        np.savez('/Users/jungseyoon/Lmser-pix2seq/dataset/airplane.npz', test=reshaped_list)
        ...
        if input_n >= result_n: # input의 stroke 개수 >= result stroke 개수
            print("LMSER 실패")
        else:
            ...
            # input의 stroke와 reshaped의 stroke concatenate
            output = np.concatenate((input_data, reshaped), axis=1)

            np.savez(f"/Users/jungseyoon/Lmser-pix2seq/result/first/{a}.npz", test=output)
